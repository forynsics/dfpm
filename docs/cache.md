...
Artifacts dfpm installs are verified and kept in a content-addressed cache, named by their SHA-256. `dfpm download` is the exception: it saves the file where you asked and caches nothing. Keeping the rest is worth the disk for four reasons:

- **Offline and air-gapped installs.** Populate the cache on a connected machine, copy the directory to an isolated one, and install there with no network.
...
- **Reinstalling needs no network.** Going back to an earlier release, or putting a version back after removing it, uses bytes already checked against their digest.
- **Provenance.** The cached file is the bytes that were verified and installed.
