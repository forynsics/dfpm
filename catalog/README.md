...
4. Inspect the archive to determine `strip_components`, the real entrypoint paths, and the files worth health checking.
...
- The archive holds `yara64.exe` and `yarac64.exe` at its root with no wrapping directory, so `strip_components` is `0`.
