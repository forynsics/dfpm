...
- **Installed and inspected, not executed.** All five install cleanly and their support directories land in the right place, verified on disk. None has been run: the review machine has no .NET runtime, and dfpm correctly refuses to launch them rather than pretending otherwise. Running each one is still outstanding.
