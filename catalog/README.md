...
5. Write `about`, and classify the package with `solves` and `artifacts`. The one-line `description` says what the tool is; `about` says why someone would reach for it, in the words an investigator would use rather than the project's own marketing. Classification is what makes the package findable by anyone who does not already know its name — which is most people, most of the time. If no existing term fits, add one to `src/dfpm/classification.py` in the same change rather than forcing a near-miss.
