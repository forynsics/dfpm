...
CAPABILITIES = _index(
    ...
    Term("volume-mounting", "Volume mounting",
         ("mount", "mounting", "attach volume", "expose volume")),
...
EVIDENCE = _index(
    ...
    Term("user-access-logging", "User access logging",
         ("ual", "user access logging", "sum", "software usage metrics", "systemidentity")),
    Term("windows-timeline", "Windows Timeline",
         ("windows timeline", "activitiescache", "activities cache", "connected devices platform")),
    Term("scheduled-tasks", "Scheduled tasks",
         ("scheduled task", "task scheduler", "at job")),
    Term("web-server-logs", "Web server logs",
         ("iis", "web server log", "access log", "http log", "w3c log")),
