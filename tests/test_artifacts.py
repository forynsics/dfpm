...
from dfpm.artifacts import acquire
from dfpm.errors import VerificationError
from dfpm.manifest import Artifact
...
class ArtifactTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.artifact = Artifact(str(self.source), hashlib.sha256(PAYLOAD).hexdigest(), len(PAYLOAD))
    ...
    def test_copies_and_verifies_a_local_path(self) -> None:
        cached = acquire(self.artifact, str(self.source), self.storage)
        self.assertEqual(cached.name, self.artifact.sha256)
        self.assertEqual(cached.read_bytes(), PAYLOAD)

    def test_accepts_a_file_url(self) -> None:
        cached = acquire(self.artifact, self.source.as_uri(), self.storage)
        self.assertEqual(cached.read_bytes(), PAYLOAD)

    def test_rejects_a_digest_mismatch_and_leaves_no_partial_behind(self) -> None:
        wrong = Artifact(str(self.source), "0" * 64, len(PAYLOAD))
    ...
    def test_rejects_a_size_mismatch(self) -> None:
        wrong = Artifact(str(self.source), self.artifact.sha256, len(PAYLOAD) + 1)
        with self.assertRaises(VerificationError) as caught:
            acquire(wrong, str(self.source), self.storage)
    ...
    def test_rejects_an_unsupported_scheme(self) -> None:
        with self.assertRaises(VerificationError) as caught:
            acquire(self.artifact, "ftp://example.org/payload.zip", self.storage)
        self.assertIn("HTTPS", str(caught.exception))

    def test_reuses_a_cached_artifact_without_the_source(self) -> None:
        acquire(self.artifact, str(self.source), self.storage)
        self.source.unlink()
        self.assertEqual(acquire(self.artifact, str(self.source), self.storage).read_bytes(), PAYLOAD)
    ...
    def test_downloads_over_https(self) -> None:
        ...
        with mock.patch("urllib.request.urlopen", return_value=FakeResponse(PAYLOAD, url)):
            cached = acquire(self.artifact, url, self.storage)
        self.assertEqual(cached.read_bytes(), PAYLOAD)
    ...
    def test_refuses_an_https_download_that_redirects_to_plain_http(self) -> None:
        ...
        with mock.patch("urllib.request.urlopen", return_value=redirected):
            with self.assertRaises(VerificationError) as caught:
                acquire(self.artifact, "https://example.org/payload.zip", self.storage)
